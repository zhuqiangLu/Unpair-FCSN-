'''
This code can be found in http://lear.inrialpes.fr/people/potapov/med_summaries.php
...
def gen_data(n, m, d=1):
    ...
    for k in range(m+1):
        ...
        X[int(cps[k]):int(cps[k+1]), :] = mus[k, :][np.newaxis, :] + \
            np.random.rand(int(cps[k+1]-cps[k]), d)
